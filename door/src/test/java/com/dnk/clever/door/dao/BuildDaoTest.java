...
public class BuildDaoTest extends BaseTest {
	...
	public void addList() {
		...
		for (int i = 1; i <= 5; i++) {
			Build build = new Build(null, "监狱" + i, 200 + i, null, null);
			builds.add(build);
		}
...
//		buildDao.findList("监狱", -1, 3).forEach(System.out::println);
		buildDao.findList("监狱", -1, 2, 2).forEach(System.out::println);
	}

	@Test
	public void deleteList() {
		List<Build> builds = buildDao.findList("监狱", 1, 1, 3);
	...
	@Test
	public void count() {
		int count = buildDao.countList(null, 203);
		System.out.println(count);
	}
}